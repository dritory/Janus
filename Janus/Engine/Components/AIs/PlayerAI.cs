...
{
    [Serializable]
    public class PlayerAI : Components.AI
    {
        [NonSerialized]
        ...
        public static Dictionary<char, string> controls = new Dictionary<char, string>() { { 'g', "Grab" }, { 'T', "Dig" }, { 'o', "Open" }, { 'c', "Close" }, { 'i', "Inventory" }, { 'h', "Heal" }, { 'u', "Use" } };
            ...
            if (!inventory)
            {
                ...
                if (dx != 0 || dy != 0)
                {
                    ...
                    if (x != 0 || y != 0)
                        ...
                        {
                            Program.engine.map.updateDynFov = true;
